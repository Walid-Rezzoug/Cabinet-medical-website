...
function genererCalendrier($mois, $annee, $conn,$idp) {
    ...
    for ($heure = 8; $heure < 16; $heure++) {
        for ($minuteOffset = 0; $minuteOffset < 60; $minuteOffset += 30) {
            ...
            if ($count == 0) {
                ...
                echo "<a href='confirmrdv.php?annee=$anneeh&mois=$moish&jour=$jourh&heure=$heureFormattee&idp=$idp' class='bouton-heure-split'>$heureFormattee</a>";
            }
        }
    }
    echo "</div>"; 
    ...
    echo "<a href='?mois=" . ($mois == 1 ? 12 : $mois - 1) . "&annee=" . ($mois == 1 ? $annee - 1 : $annee) . "&idp=$idp' class='fleche-split fleche-gauche-split'>&lt;</a>";
    echo "<div class='mois-annee-split'>" . date('M', strtotime("$annee-$mois-01")) . " " . $annee . "</div>";
    echo "<a href='?mois=" . ($mois == 12 ? 1 : $mois + 1) . "&annee=" . ($mois == 12 ? $annee + 1 : $annee) . "&idp=$idp' class='fleche-split fleche-droite-split'>&gt;</a>";
    echo "</div>"; 
    ...
    for ($jour = 1; $jour <= $nombreJoursDansMois; $jour++) {
        ...
        $boutonContenu = $estJourPasse ? "<div class='bouton-rond-split desactive-split'>$jour</div>" : "<a href='nvrdv1.php?annee=$annee&mois=$mois&jour=$jour&idp=$idp' class='bouton-rond-split'>$jour</a>";
...
genererCalendrier($moisAffiche, $anneeAffiche, $conn,$idp);
$conn->close();
