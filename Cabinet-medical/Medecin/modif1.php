...
function genererCalendrier($mois, $annee, $conn,$idr) {
    ...
    for ($heure = 8; $heure < 16; $heure++) {
        for ($minuteOffset = 0; $minuteOffset < 60; $minuteOffset += 30) {
            ...
            if ($count == 0) {
                $idr = $_GET['idr'];
                echo "<a href='modifrdv.php?annee=$anneeh&mois=$moish&jour=$jourh&heure=$heureFormattee&idr=$idr' class='bouton-heure-split'>$heureFormattee</a>";
            }
        }
    }
    echo "</div>"; 
    ...
    echo "<a href='?mois=" . ($mois == 1 ? 12 : $mois - 1) . "&annee=" . ($mois == 1 ? $annee - 1 : $annee) . "&idr=$idr' class='fleche-split fleche-gauche-split'>&lt;</a>";
    echo "<div class='mois-annee-split'>" . date('M', strtotime("$annee-$mois-01")) . " " . $annee . "</div>";
    echo "<a href='?mois=" . ($mois == 12 ? 1 : $mois + 1) . "&annee=" . ($mois == 12 ? $annee + 1 : $annee) . "&idr=$idr' class='fleche-split fleche-droite-split'>&gt;</a>";
    echo "</div>"; 
    ...
    for ($jour = 1; $jour <= $nombreJoursDansMois; $jour++) {
        ...
        $boutonContenu = $estJourPasse 
            ...
            : "<a href='modif1.php?annee=$annee&mois=$mois&jour=$jour&idr=$idr' class='bouton-rond-split'>$jour</a>";
...
genererCalendrier($moisAffiche, $anneeAffiche, $conn,$idr);
$conn->close();
