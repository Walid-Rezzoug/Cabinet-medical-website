...
if (!isset($_SESSION['mama'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] != 'POST'){
        ...
        function genererCalendrier($mois, $annee,$idp) {
            ...
            echo "<a href='?mois=" . ($mois == 1 ? 12 : $mois - 1) . "&annee=" . ($mois == 1 ? $annee - 1 : $annee) . "&idp=$idp' class='fleche-moitie fleche-gauche-moitie'>&lt;</a>";
            echo "<div class='mois-annee-moitie'>" . date('M', strtotime("$annee-$mois-01")) . " " . $annee . "</div>";
            echo "<a href='?mois=" . ($mois == 12 ? 1 : $mois + 1) . "&annee=" . ($mois == 12 ? $annee + 1 : $annee) . "&idp=$idp' class='fleche-moitie fleche-droite-moitie'>&gt;</a>";
            echo "</div>";
            ...
            for ($jour = 1; $jour <= $nombreJoursDansMois; $jour++) {
                ...
                $boutonContenu = $estJourPasse ? "<div class='bouton-rond-moitie desactive-moitie'>$jour</div>" : "<a href='nvrdv1.php?annee=$annee&mois=$mois&jour=$jour&idp=$idp' class='bouton-rond-moitie' onclick='afficherDate(this, $jour, $mois, $annee)'>$jour</a>";
            ...
            echo "</div>";
            echo "</div>";
            echo "<a href='select.php'> <button type='button' class='button3' >Back</button></a>";
        }
?>
...
.bouton-rond-moitie.desactive-moitie {
    ...
    <div class="container2">
        ...
        genererCalendrier($moisAffiche, $anneeAffiche,$idp);
            ?>
    </div>
