...
if (isset($_GET['id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        function genererCalendrier($mois, $annee,$id,$idp) {
            ...
            echo "<a href='?mois=" . ($mois == 1 ? 12 : $mois - 1) . "&annee=" . ($mois == 1 ? $annee - 1 : $annee) . "&id=$id&idp=$idp' class='fleche-moitie fleche-gauche-moitie'>&lt;</a>";
            echo "<div class='mois-annee-moitie'>" . date('M', strtotime("$annee-$mois-01")) . " " . $annee . "</div>";
            echo "<a href='?mois=" . ($mois == 12 ? 1 : $mois + 1) . "&annee=" . ($mois == 12 ? $annee + 1 : $annee) . "&id=$id&idp=$idp' class='fleche-moitie fleche-droite-moitie'>&gt;</a>";
            echo "</div>";
            ...
            for ($jour = 1; $jour <= $nombreJoursDansMois; $jour++) {
                ...
                $boutonContenu = $estJourPasse ? "<div class='bouton-rond-moitie desactive-moitie'>$jour</div>" : "<a href='nvrdv1.php?annee=$annee&mois=$mois&jour=$jour&id=$id&idp=$idp' class='bouton-rond-moitie' onclick='afficherDate(this, $jour, $mois, $annee)'>$jour</a>";
            ...
            echo "</div>";
            echo "</div>";
            echo "<a href='select.php?id=$id'> <button type='button' class='button3' >Back</button></a>";
        }
?>
...
.bouton-rond-moitie.desactive-moitie {
    ...
    <div class="container2">
        ...
        genererCalendrier($moisAffiche, $anneeAffiche,$id,$idp);
            ?>
    </div>
...
<?php
    ...
    echo "<h3>Error while searching the id <a href='select.php'><button>Back</button></a></h3></script>";
}
$conn->close();
?>